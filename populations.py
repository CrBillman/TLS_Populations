...
class Population:
    ...
    def UpdateEquilibrium(self):
        ...
        try:
            self.equilibrium = self.dipole * (self.delta / E) * np.tanh( E / ( 2 * self.temp ))
        except ZeroDivisionError:
            self.equilibrium = - self.dipole * (self.delta / E)
        return

    def TakeStep(self, deltaT = 0.1):
        self.UpdatePolarizability()
        instantEquilibrium = self.equilibrium + self.alpha * self.field
        diff = self.population - instantEquilibrium
        if diff >= 0.0:
            partial = - diff / self.Tau1
        else:
            partial = - diff/ self.Tau2
    ...
    def SetToEquilibrium(self):
        self.population = self.equilibrium
        return
